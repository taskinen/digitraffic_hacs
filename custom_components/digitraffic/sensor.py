...
from .translations import translate_sensor_value
...
class DigitrafficWeatherSensor(CoordinatorEntity[DigitrafficDataUpdateCoordinator], SensorEntity):
    ...
    def __init__(
        ...
        # Override state_class for translated sensors (categorical, not measurements)
        if self._config.get("translate", False):
    ...
    def native_value(self) -> Any | None:
        """Return the state of the sensor, translated if applicable."""
        if self.coordinator.data and self._sensor_key in self.coordinator.data:
            value = self.coordinator.data[self._sensor_key]

            # Check if this sensor should be translated
            if self._config.get("translate", False):
                return translate_sensor_value(self._sensor_key, value)
            else:
                # Attempt to convert to float for numeric sensors
                try:
                    return float(value)
    ...
    def extra_state_attributes(self):
        ...
        if self.coordinator.data:
            if "measuredTime" in self.coordinator.data:
                attrs["measured_time"] = self.coordinator.data["measuredTime"]

            # Add raw value for translated sensors
            if self._config.get("translate", False) and self._sensor_key in self.coordinator.data:
                raw_value = self.coordinator.data[self._sensor_key]
                try:
                    attrs["raw_value"] = float(raw_value)
                except (ValueError, TypeError):
